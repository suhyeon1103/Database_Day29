...
SELECT s.student_id, s.name AS student_name, c.club_name
FROM students s
CROSS JOIN clubs c
LEFT JOIN club_memberships cm ON s.student_id = cm.student_id AND c.club_id = cm.club_id
...
SELECT name FROM customers WHERE customer_id IN (SELECT customer_id FROM subscribers)
INTERSECT
SELECT name FROM customers WHERE customer_id IN (SELECT customer_id FROM subscribers);
...
EXCEPT
SELECT name FROM students
JOIN graduates ON students.student_id = graduates.student_id;
